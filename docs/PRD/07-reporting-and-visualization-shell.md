...
**Sample reports (added after Stage 8 made real matches possible):** `scripts/generate_sample_reports.py` plays an actual match and writes all four mandatory JSON report types to `docs/sample_reports/` -- real data, not fabricated; the log passes `ReplayViewer.verify_all()`. This satisfies the "four example JSON files... illustrating the full usage format" the book itself attaches (Sec. 9.3.3), but is not a real league submission -- that still needs your own Gmail credentials and an actual opponent.
